...
Window:AddButton({
    Title = "Get Key",
    Description = "Nhấn để nhận key và copy Discord",
    Callback = function()
        local key = "orenzoiun5"
        local discordLink = "https://discord.gg/cwsTyeuB"

        if setclipboard then
            setclipboard("Key: "..key.." | Discord: "..discordLink)
        end

        Fluent:Notify({
            Title = "Key đã copy!",
            Content = "Key: "..key.."\nDiscord đã copy vào clipboard.",
            Duration = 5
        })
    end
})
