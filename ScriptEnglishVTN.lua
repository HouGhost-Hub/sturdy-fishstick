...
Window = Fluent:CreateWindow({
    Title = "Van-Nguyen Hub [Fremium] Buy xibasexh6",
...
wait(1.0)
Tabs.Info:AddButton({
        Title="Nhóm Discord Van Nguyen Community Hub",
        Description="Discord",
        Callback=function()
...
Tabs.Info:AddButton({
    Title="Youtube Tên Văn Nguyên Nhớ đăng Ký cho mình",
    Description="Youtube",
    Callback=function()
...
if executorName then
    Tabs.Info:AddParagraph({
        Title="Client Hack Các Bạn Đang Sự dụng",
        Content=executorName
    })
end
Tabs.Info:AddParagraph({
    Title="Update nên chưa hỗ trợ",
...
function Tween(Pos)
    pcall(function()
        ...
        local tweeninfo = TweenInfo.new((Pos.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude / 300, Enum.EasingStyle.Linear)
        local tween = tween_s:Create(game.Players.LocalPlayer.Character.HumanoidRootPart, tweeninfo, {CFrame = Pos})
        tween:Play()
...
function CheckLevel()
    ...
    QuestLv = 1
    Ms = "Bandit [Lv. 5]"
    CFrameQ = CFrame.new(1060, 17, 1612)
end
    ...
    while task.wait() do
        if _G.AutoLevel then
            pcall(function()
                ...
                local gui = plr.PlayerGui.Main.Quest
                ...
                if not gui.Visible or not string.find(gui.Container.QuestTitle.Title.Text, NameMon) then
                    Notify("Trạng thái", "Nhận lại nhiệm vụ")
                        ...
                        if v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then
                            if v.Name == Ms then
                                ...
                                if count <= 20 then
                                    ...
                                    v.HumanoidRootPart.CFrame = hrp.CFrame * CFrame.new(0,0,-7)
                                end
                            end
                        end
                    end
                    ...
                    if not hasMob then
                        Notify("Trạng thái", "Tìm vị trí quái mới")
                        for _, sp in pairs(workspace["_WorldOrigin"].EnemySpawns:GetChildren()) do
                            if string.find(sp.Name, NameMon) then
                                Tween(sp.CFrame * CFrame.new(0,10,0))
                                break
                            end
                        end
                    else
                        Notify("Trạng thái", "Đang farm (" .. tostring(count) .. " quái)")
                    end
...
local ToggleAwake = Tabs.Raid:AddToggle("ToggleAwake", {Title="Awaken",Description="", Default=false })
...
local notificationCooldown = 0.1
