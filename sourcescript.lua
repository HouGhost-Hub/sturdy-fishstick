...
v5.Window = function(v134, v135)
	...
	v170.Image = "http://www.roblox.com/asset/?id="   .. tostring(88751651734125) ;
...
local currentMode = "Xeter Attack"
local attackDelay = attackModes[currentMode]
...
for _,v in pairs(game:GetDescendants()) do
	if v:IsA("RemoteEvent") then
		...
		if string.find(name, "attack") or string.find(name, "combat") or string.find(name, "melee") then
			foundRemote = v
			break
		end
	end
...
v68:Dropdown("Fast Attack Delay", {"Normal Attack", "Fast Attack", "Super Attack", "Xeter Attack"}, function(selected)
	currentMode = selected
	attackDelay = attackModes[selected]
end)

v68:Toggle("Auto Attack", false, function(state)
	autoAttack = state
end)
...
local function attack()
	...
	pcall(function()
		foundRemote:FireServer("Attack", true)
	end)
end
