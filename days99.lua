...
local Window = Rayfield:CreateWindow({
   Name = "HouGhost Hub Preumium",
   Icon = 0, -- Icon in Topbar. Can use Lucide Icons (string) or Roblox Image (number). 0 to use no icon (default).
   LoadingTitle = "99 Nights In The Forest",
   LoadingSubtitle = "Script By orenzoiun",
...
local PlayerTab = Window:CreateTab("Settings")
local EspTab = Window:CreateTab("Esp")
local GameTab = Window:CreateTab("Combat")
local BringItemTab = Window:CreateTab("Bring Item")
local DiscordTab = Window:CreateTab("Discord")
local SettingsTab = Window:CreateTab("Misc")
...
local DistanceForKillAura = 0
local DistanceForAutoChopTree = 0
Rayfield:Notify({
   Title = "Cheat Version",
   Content = "V.1.0",
   Duration = 2.5,
...
local DiscordLink = DiscordTab:CreateButton({
   Name = "Discord Link",
   Callback = function()
copyToClipboard("https://discord.gg/nradqyeA")
end,
})
...
for _, bunny in pairs(workspace.Characters:GetChildren()) do
	if bunny:IsA("Model") and bunny.PrimaryPart then
		...
		if distance <= DistanceForKillAura then
			local result = game:GetService("ReplicatedStorage").RemoteEvents.ToolDamageObject:InvokeServer(bunny, weapon, 500, hrp.CFrame)
		end
	end
end
end)
